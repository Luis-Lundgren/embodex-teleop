...
AFRAME.registerComponent('controller-updater', {
  init: function () {
        ...
        try {
          ...
          if (data.type === 'robot_state') {
            ...
            if (data.left_arm && data.left_arm.length >= 6) {
              ...
              const angles = data.left_arm; // Assuming degrees

              // Map joints to entities and axes
              // J1: 0 -1 0 -> y = -angle
              const j1 = document.querySelector('#left-j1');
              if (j1) j1.object3D.rotation.y = THREE.MathUtils.degToRad(-angles[0]);

              // J2: 1 0 0 -> x = angle
              const j2 = document.querySelector('#left-j2');
              if (j2) j2.object3D.rotation.x = THREE.MathUtils.degToRad(angles[1]);

              // J3: 1 0 0 -> x = angle
              const j3 = document.querySelector('#left-j3');
              if (j3) j3.object3D.rotation.x = THREE.MathUtils.degToRad(angles[2]);

              // J4: 1 0 0 -> x = angle
              const j4 = document.querySelector('#left-j4');
              if (j4) j4.object3D.rotation.x = THREE.MathUtils.degToRad(angles[3]);

              // J5: 0 1 0 -> y = angle
              const j5 = document.querySelector('#left-j5');
              if (j5) j5.object3D.rotation.y = THREE.MathUtils.degToRad(angles[4]);

              // J6: 0 0 1 -> z = angle (Gripper)
              const j6 = document.querySelector('#left-j6');
              if (j6) j6.object3D.rotation.z = THREE.MathUtils.degToRad(angles[5]);
            }
          }
        } catch (e) {
    ...
    this.leftHand.addEventListener('xbuttondown', (evt) => {
      console.log('Left X Button Pressed - Toggling Recording');
      if (this.websocket && this.websocket.readyState === WebSocket.OPEN) {
        this.websocket.send(JSON.stringify({ action: 'record_toggle' }));
      }
    });
